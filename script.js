...
const towers = {
    ...
    update: function () {
        ...
        if (frames % 100 === 0) {
            ...
            // Constrain height difference to ensure playability
            if (this.list.length > 0) {
                const prevTop = this.list[this.list.length - 1].top;
                // Allow max 200px change up or down
                const maxDiff = 200;

                const safeMin = Math.max(minTop, prevTop - maxDiff);
                const safeMax = Math.min(maxTop, prevTop + maxDiff);

                topHeight = Math.random() * (safeMax - safeMin) + safeMin;
            } else {
                // First tower: completely random
