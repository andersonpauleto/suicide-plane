...
const towers = {
    list: [],
    width: 60, // Tower width
    dx: 3,     // Speed (Reverted to 3 for Mobile)
    gap: 150,  // Gap between top and bottom towers
    spawnTimer: 0,
    ...
    draw: function () {
        for (let t of this.list) {
            ...
            if (towerImg.complete && towerImg.naturalWidth !== 0) {
                // We need to tile or stretch the tower image?
                // Standard Flappy Bird uses repetitive pipes. 
                // We can just draw the image stretched or clipped. 
                // Let's stretch for simplicity or fill pattern.
                ctx.drawImage(towerImg, t.x, t.top - 600, this.width, 600); // Draw image shifted up so bottom is at t.top
                ctx.drawImage(towerImg, t.x, t.bottom, this.width, 600); // Draw image from top down
            } else {
                ctx.fillStyle = '#555';
                ctx.fillRect(t.x, 0, this.width, t.top);
                ctx.fillRect(t.x, t.bottom, this.width, canvas.height - t.bottom);

                // Add "windows" detail
                ctx.fillStyle = '#777';
                ctx.fillRect(t.x + 10, t.top - 30, 10, 20);
                ctx.fillRect(t.x + 40, t.top - 30, 10, 20);
                ctx.fillRect(t.x + 10, t.bottom + 10, 10, 20);
                ctx.fillRect(t.x + 40, t.bottom + 10, 10, 20);
            }
        }
    },
    ...
    update: function () {
        ...
        // Spawn
        if (frames % 100 === 0) { // Approx every 1.5-2 seconds
            // Random position
            // The gap should be within the viewable area
            // min top height: 50, max top height: canvas.height - gap - 50
        ...
        for (let i = 0; i < this.list.length; i++) {
            ...
            // Plane hitbox is slightly smaller than sprite
            const pHit = {
                x: plane.x + 5,
                y: plane.y + 5,
                w: plane.w - 10,
            ...
            // Score
            if (t.x + this.width < plane.x && !t.passed) {
                score += 10;
                scoreEl.innerHTML = `Pontos: ${score}`;
                t.passed = true;
                // Check if we need to increase difficulty? 
                // User asked for specific score increments.
            }
...
function loop() {
    ...
    if (gameState === 'PLAYING') {
        ...
        // Intro / Menu
        plane.update();
        towers.update(); // Safe because it has guard clause now
    }
    ...
    if (gameState !== 'GAMEOVER') plane.draw(); // Plane drawn separately in GAMEOVER
...
// Init
resize();
loop(); // Start the game loop (Intro)
